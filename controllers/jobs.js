...
function getByKeywords(req, res) {
    ...
    var langRegex = parser.getLangRegex(req.query.lang);
    Job.paginate({ title: { $regex: regex }, language: { $regex: langRegex } }, {
...
function getWithoutKeywords(req, res) {
    var langRegex = parser.getLangRegex(req.query.lang);
    Job.paginate({ language: { $regex: langRegex } }, {
            select: 'title location company type languages',
            sort: { 'postDate': -1 },
            offset: req.query.skip * 1,
            limit: req.query.get * 1
        },
...
module.exports = (function() {
    ...
    api.route('/joblangs')
        .get(function(req, res) {
            Job.distinct('language', function(err, data) {
                if (err) res.send(err);
                console.log(err);
